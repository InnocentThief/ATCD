...
class PublishedSongList extends React.Component {
    async componentDidMount() {
        const {
            account: { fetchPublishedSongs }
        } = Context
    ...
    render() {
        const {
            account: { loadedPublishedSongs }
        } = Context
        return (
            <Container>
                {loadedPublishedSongs.length === 0 && (
                    <NonIdealState
                        icon="music"
                        iconSize={NonIdealStateIconSize.SMALL}
                        title="No published songs found"
                        description="There are no published songs yet"
                    />
                )}
                {loadedPublishedSongs.length > 0 && (
                    <SongList>
                        {loadedPublishedSongs.map(song => (
                            <SongCard>
                                ...
                                <SongInfo vertical={true}>
                                    ...
                                    <Tooltip2
                                        ...
                                        compact={true}
                                    >
                                        <Button minimal={true} icon="eye-off" intent="primary" />
                                    </Tooltip2>
                                </SongInfo>
                            </SongCard>
                        ))}
...
export default observer(PublishedSongList)