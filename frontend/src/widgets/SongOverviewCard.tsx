...
const SongOverviewCard = ({
    ...
    return (
        <SongCard>
            ...
            <SongCardSongInfo>
                <H5>
                    <Link to={{ pathname: `songs/${item.songKey}` }}>{item.title} - {item.artist}</Link>
                </H5>
                <p>Mapped by <Link to={{ pathname: `mappers/${item.authorKey}` }}>{item.author}</Link></p>
            ...
            <SongCardAdditionalInfo>
                ...
                <SongCardAdditionalInfoRow>
                    <SongCardAdditionalInfoValue>{item.length}</SongCardAdditionalInfoValue>
                    <Icon icon="time" />
                </SongCardAdditionalInfoRow>
                <SongCardAdditionalInfoRow>
                    <SongCardAdditionalInfoValue>{item.avgBpm}</SongCardAdditionalInfoValue>
                    <Icon icon="dashboard" />
                </SongCardAdditionalInfoRow>
                <SongCardAdditionalInfoRow>
                    {item.contentStrike && (
            ...
            <SongCardActions>
                <Tooltip2 content={get('Songs.Action.CopyAtr')} placement="top" compact={true}>
                        <Button minimal={true} icon="duplicate" intent='primary' onClick={() => copyATR(item.atr)} />
                    </Tooltip2>
